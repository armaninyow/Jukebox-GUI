...
public class JukeboxManagementScreen extends HandledScreen<JukeboxScreenHandler> {
	...
	// Slot vanilla highlight textures
	private static final Identifier SLOT_HIGHLIGHT_BACK =
		Identifier.of("minecraft", "textures/gui/sprites/slot_highlight_back.png");
	...
	protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
		...
		// ── Progress bar area ──────────────────────────────────────────────
		// Background
		context.drawTexture(pipeline, PROGRESS_BG_TEXTURE,
		...
		if (!disc.isEmpty() && totalSeconds > 0) {
			...
			if (progTex != null) {
				...
				if (progWidth > 0) {
					...
					context.drawTexture(pipeline, progTex,
						gx + PROG_X, gy + PROG_Y,
						0f, 0f,
						progWidth, PROG_H,
						progWidth, PROG_H,
