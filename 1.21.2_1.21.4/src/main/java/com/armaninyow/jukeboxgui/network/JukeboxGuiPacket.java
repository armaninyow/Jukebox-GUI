...
public class JukeboxGuiPacket {
	...
	public static void sendToClient(ServerPlayerEntity player, JukeboxBlockEntity jukebox, BlockPos pos) {
		...
		// Read ticks_since_song_started — use contains() check for pre-1.21.5 compat
		// (two-arg getLong with default was added in 1.21.5)
		NbtCompound nbt = jukebox.createNbt(registries);
		long ticksSinceSongStarted = nbt.contains("ticks_since_song_started")
			? nbt.getLong("ticks_since_song_started") : -1L;
